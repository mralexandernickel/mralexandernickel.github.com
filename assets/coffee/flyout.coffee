$ ->
  ...
  $("#search_field").keyup (e) ->
    ...
    if search_str.length > 1
      ...
      # show result to user
      for item in result
        search_results.append $("<div class=\"span4\"><article class=\"teaser\"><header><h3>#{item.title}</h3><h4><em>#{item.author} <small>#{item.date_string}</small></em></h4></header><div class=\"content_wrap\"><p>#{item.content}</p></div><a class=\"read_more\" href=\"#{item.href}\">READ MORE</a></article></div>")
